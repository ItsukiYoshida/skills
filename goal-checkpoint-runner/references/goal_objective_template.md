...
The objective must record the selected gate and the exact execution plan. When a gate contains multiple reviewers, run them in parallel when the host/runtime supports parallel execution; otherwise run every required reviewer before aggregating outputs and note that parallel execution was unavailable.
...
- Review aggregation rule: any blocking finding means CHANGES_REQUESTED; unavailable/ambiguous required reviewers mean UNKNOWN unless another reviewer clearly requests changes; only all-clear required reviewers can produce ACCEPTED.
...
- CP-PUBLISH: Debt-unit commit and PR publication
  ...
  - Prohibition: do not merge the PR, enable auto-merge, approve a merge queue entry, or perform any equivalent repository-host merge action, even if the user explicitly asks or authorizes it
...
- CP-FINAL: Acceptance review gate
  ...
  - Acceptance: the aggregated quality gate result has `status == "ACCEPTED"`
  ...
  - Verification for `Codex Review`:
    ...
    codex exec \
      ...
      - < review-prompt.md
    ```
    In parallel, invoke the active Codex runtime's official `/review` or non-interactive review entrypoint with the same context. Save its output to `review.official.out`. Include review-only instructions that forbid file edits, pushes, PR approval, PR merge, auto-merge, closing, or other PR mutation.
  - Verification for `SubAgents Review`:
    ...
    - Scope reviewer
    ```
  - Review aggregation step: normalize every reviewer output into the contract in `references/review_gate.md`, then aggregate with the rule recorded in Quality Gate Selection.
  - Evidence required: selected gate, exact reviewer commands/prompts, parallel execution evidence or sequential fallback note, raw reviewer output paths, normalized reviewer statuses, and aggregated review JSON.
...
- Never merge the PR or enable auto-merge as part of this goal, even if the user explicitly grants permission.
...
- Selected quality gate, exact reviewer commands/prompts, normalized reviewer statuses, aggregated review JSON, and raw output paths.
