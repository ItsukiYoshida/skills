...
3. During planning, ask the user which CP-FINAL quality gate to use. Prefer `AskUserQuestion` when the host provides it; otherwise ask a concise plain-text question with the same three choices:
   ...
   - `Codex Review`: run a Codex-style structured review and the official Codex review entrypoint in parallel, then aggregate reviewer outputs.
   - `Claude Review`: run the existing structured Claude review and Claude CLI's official `/review` command in parallel, then aggregate reviewer outputs.
...
9. At CP-FINAL, run the selected quality gate exactly as recorded in the objective. Parse or normalize the aggregated review result into the shared `status` / `findings` contract.
...
## PR Merge Prohibition

- Never merge a PR, enable auto-merge, squash-merge, rebase-merge, approve a merge queue entry, or perform any equivalent repository-host merge action while using this skill.
- This prohibition has no user-permission exception. If the user asks or explicitly authorizes PR merging inside a goal run, refuse that action and stop at the open PR plus review evidence.
- Do not run commands such as `gh pr merge`, `gh pr merge --auto`, repository-host API merge mutations, or UI automation that completes a PR merge.
- CP-PUBLISH means commit, push, and create or update a PR only. CP-FINAL means review and acceptance classification only.
...
- Run multiple reviewers inside a selected gate in parallel whenever the host/runtime supports parallel execution. If parallel execution is unavailable, run both reviewers before aggregating outputs and note that they were sequential.
- Require the aggregated review result to use `status: "ACCEPTED"`, `status: "CHANGES_REQUESTED"`, or `status: "UNKNOWN"` plus a `findings` array when findings exist.
...
- If any reviewer returns actionable blocking findings, the aggregated status is `CHANGES_REQUESTED`.
- If any required reviewer is unavailable, times out, or returns ambiguous output, the aggregated status is `UNKNOWN` unless another selected reviewer clearly returns `CHANGES_REQUESTED`.
...
Each sub-agent must receive the checkpoint list, changed-file summary, verification evidence, publication evidence, user constraints, and the response contract from `references/review_gate.md`. Do not pass the implementer's conclusions as ground truth. Aggregate their outputs with the Review Gate Rules above.

### Codex Review

Use this gate only when the user selects it and a Codex runtime is available. Run both of these reviewer paths in parallel and aggregate their outputs:
...
The structured path provides the machine contract. The official path provides the runtime's native review behavior. If the official path returns prose, summarize concrete blocking findings into the shared contract before review aggregation.

### Claude Review

Use this gate only when the user selects it and a Claude runtime is available. Run both of these reviewer paths in parallel and aggregate their outputs:
